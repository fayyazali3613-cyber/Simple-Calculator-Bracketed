...
function loadFromStorage() {
    ...
    if (wasFocused && html.length > 0) {
        setTimeout(() => {
            const plainExpr = expression;
const formattedText = inputDisplay.innerText.replace(/\n/g, '');

const visualPos = mapRawPosToFormatted(
    plainExpr,
    formattedText,
    cursorPosition
);

setCursorPosition(visualPos);
    ...
    /* ===============================
       HELPER FUNCTIONS
    =============================== */
	function mapRawPosToFormatted(rawExpr, formattedText, rawPos) {
    let rawCount = 0;
    let formattedPos = 0;

    while (formattedPos < formattedText.length && rawCount < rawPos) {
		if (/[0-9.\-]/.test(formattedText[formattedPos])) {
    rawCount++;
}
    ...
    // Wrap negative numbers after operators
    expr = expr.replace(/([\*\/\+])-([0-9])/g, '$1(-$2');

    // Close missing brackets from wrapping
...
=============================== */
    ...
    if (['+', '-', '*', '/'].includes(lastChar)) {
        ...
        // Agar lastAnswer negative hai
        if (lastAnswer.startsWith('-')) {
            expression += "(" + lastAnswer + ")";
            cursorPosition = expression.length;
            render();
            return;
    ...
    console.log("Before backspace:", { 
        ...
        charBefore: cursorPosition > 0 ? expression[cursorPosition - 1] : 'none'
    });
    
    if (cursorPosition > 0) {
        // Check if we're at power operator
        if (cursorPosition >= 2 && 
            expression.substring(cursorPosition - 2, cursorPosition) === "**") {
            console.log("Deleting power operator");
            ...
            console.log("Deleting sqrt");
            expression = expression.slice(0, cursorPosition - 1) + expression.slice(cursorPosition);
            cursorPosition--;
        }
        else {
            console.log("Deleting normal character:", expression[cursorPosition - 1]);
            expression = expression.slice(0, cursorPosition - 1) + expression.slice(cursorPosition);
            cursorPosition--;
